...
export function CandidatePlayer() {
  ...
  // Fallback to fetch random questions from catalog since attempt questions endpoint isn't fully exposed
  const { data: questions = [], isFetching } = useQuery({
    queryKey: ["candidate-questions", attemptId],
    queryFn: async () => {
      const res = await api.get(`/catalog/questions`, { params: { subject: "Java", size: 5 } });
      return res.data;
    },
  ...
  const startMutation = useMutation({
    mutationFn: async () => {
      // Typically we'd validate link first, but for demo we can jump to start attempt
      const res = await api.post(`/candidate/attempts/start/${invitationId}`);
      return res.data;
    },
    onSuccess: (data) => {
    ...
    onError: (error: any) => {
      setMessage(`Failed to start: ${error.response?.data?.message || error.message}`);
    }
  });
  ...
  const submitMutation = useMutation({
    ...
    onError: (error: any) => {
      setMessage(`Failed to submit: ${error.response?.data?.message || error.message}`);
    }
  });
          ...
          {isFetching ? (
            ...
            <>
              ...
              <div className="grid gap-3">
                {/* Fallback to mock options since API generated questions don't have structured options in summary */}
                {["ps aux | grep java", "lsof -i -P -n", "netstat -tulpn", "kill -0 <pid>"].map((choice, index) => (
