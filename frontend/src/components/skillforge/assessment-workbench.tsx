...
export function AssessmentWorkbench() {
  ...
  const publishMutation = useMutation({
    mutationFn: async () => {
      ...
      // We would normally add sections and questions here
      // 2. Publish
    ...
    onSuccess: (data) => {
      ...
      setMessage(`Published ${draft.title} with ${totalQuestions} randomized questions from the ${draft.subject} bank.`);
    },
    onError: (error: any) => {
      setMessage(`Failed to publish: ${error.response?.data?.message || error.message}`);
    }
  });
  ...
  const inviteMutation = useMutation({
    ...
    onSuccess: (data: any[]) => {
      const tokens = data.map((invite) => invite.tokenPreview || `Invite sent to ${invite.candidateUserId}`);
      setInvitations(tokens);
      setMessage(`Queued ${tokens.length} secure one-time email invitations.`);
    },
    onError: (error: any) => {
      setMessage(`Failed to invite: ${error.response?.data?.message || error.message}`);
    }
  });
